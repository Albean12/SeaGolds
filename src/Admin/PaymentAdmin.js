...
const PaymentAdmin = () => {
    ...
    const fetchMergedData = async () => {
        ...
        try {
            ...
            const [paymentsRes, unpaidRes] = await Promise.all([
                fetch(`https://backend-production-8fda.up.railway.app/api/payments?${query}`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
                fetch(`https://backend-production-8fda.up.railway.app/api/unpaid-tenants?${query}`, {
                    headers: { Authorization: `Bearer ${token}` },
                }),
            ]);
    ...
    const handleStatusUpdate = async (user_id, status) => {
        ...
        try {
            const endpoint =
                status === 'Confirmed'
                    ? `https://backend-production-8fda.up.railway.app/api/payments/confirm/${user_id}`
                    : `https://backend-production-8fda.up.railway.app/api/payments/reject/${user_id}`;
