...
const AdminTourBookings = () => {
    ...
    useEffect(() => {
        fetch('https://backend-production-8fda.up.railway.app/api/tour-bookings')
    ...
    const fetchBookings = async () => {
        try {
            const response = await fetch('https://backend-production-8fda.up.railway.app/api/tour-bookings', {
    ...
    const handleConfirmBooking = async (id) => {
        try {
            const response = await fetch(`https://backend-production-8fda.up.railway.app/api/bookings/confirm/${id}`, {
                method: 'POST',
                headers: {
    ...
    const handleCancelBooking = async (id) => {
        try {
            const response = await fetch(`https://backend-production-8fda.up.railway.app/api/bookings/cancel/${id}`, {
                method: 'POST',
                headers: {
                    Authorization: `Bearer ${localStorage.getItem('token')}`,
                },
            ...
            if (response.ok) {
                ...
                if (selectedDate) {
                    fetch(`https://backend-production-8fda.up.railway.app/api/tour-slots?date=${selectedDate}`)
                        .then((res) => res.json())
