...
const AdminGallery = () => {
  ...
  const fetchImages = async () => {
    try {
      const response = await fetch('https://backend-production-8fda.up.railway.app/api/gallery', {
  ...
  const handleImageUpload = async (e) => {
    ...
    try {
      const response = await fetch('https://backend-production-8fda.up.railway.app/api/gallery/upload', {
        method: 'POST',
        body: formData,
        headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
      });
  ...
  const handleImageDelete = async (id) => {
    ...
    try {
      const response = await fetch(`https://backend-production-8fda.up.railway.app/api/gallery/${id}`, {
        method: 'DELETE',
        headers: {
      ...
      <div className="gallery-grid">
        {images.map((img) => (
          <div key={img.id} className="gallery-card">
            <img src={`https://backend-production-8fda.up.railway.app/storage/${img.image_path}`} alt={img.title} />
